...
def compare_pt_files(input_file, output_file, output_dir, file_idx):
    ...
    if len(valid_epochs) == 0:
        # print("\nNo valid epochs to plot (all are None or zero)!")
        return
    ...
    # if zero_std_count > 0:
    #     print(f"  {zero_std_count} epochs have near-zero output std!")
...
def compare_fif_files(original_file, preprocessed_file, output_file, output_dir, file_idx, include_original_fif, normalize_for_comparison):
    ...
    # print(f"  Input in uV:  mean={data_preprocessed.mean()*1e6:.2f}, std={data_preprocessed.std()*1e6:.2f}")
    # print(f"  Output in uV: mean={data_output.mean()*1e6:.2f}, std={data_output.std()*1e6:.2f}")
...
def compare_pipeline(
    ...
    if len(pt_input_files) > 0 and len(pt_output_files) > 0:
        ...
        for pt_input in pt_input_files:
            ...
            # if none_count > 0:
            #     print(f"  {none_count} epochs filtered by model (channel count mismatch)")
