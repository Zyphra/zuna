...
#   >> CUDA_VISIBLE_DEVICES=6 python3 apps/AY2latent_bci/eeg_eval.py config=apps/AY2latent_bci/configs/config_bci_eval.yaml
...
def plot_compare_eeg(data,
    ...
    invert = True # This should be False!
    if invert:
        reconst = -reconst
    ...
    for i in range(8):
        for j in range(8):
            ...
            axes[i, j].plot(t, data[:, ch], "b-", linewidth=0.5, alpha=0.9)
            axes[i, j].plot(t, reconst[:, ch], "r-", linewidth=0.5, alpha=0.3)
    ...
    plt.suptitle(f"Normed EEG - (batch={batch}, idx={idx}, sample={sample})", fontsize=16, fontweight='bold')
...
def plot_compare_fft(data, 
    ...
    for i in range(8):
        for j in range(8):
            ...
            # Plot time-domain EEG (offset by channel index)
            axes[i, j].plot(freqs, data[:, ch], "b-", linewidth=0.5, alpha=0.9)
            axes[i, j].plot(freqs, reconst[:, ch], "r-", linewidth=0.5, alpha=0.3)
    ...
    fig.text(0.30, 0.97, "vs.", ha='center', va='center', fontsize=16, fontweight='bold', color='black')
    fig.text(0.34, 0.97, "reconst", ha='center', va='center', fontsize=16, fontweight='bold', color='red')
    plt.suptitle(f"Normed EEG FFT - (batch={batch}, idx={idx}, sample={sample})", fontsize=16, fontweight='bold')
...
def plot_eeg_fft_psd_plv(signal, 
                         ...
                         dir_base="figures"):
    """
    Plot ChannelNormed EEG signal on top,
    and FFT of ChannelNormed EEG on bottom,
...
def evaluate(args: TrainArgs):
    ...
    with ExitStack() as context_stack:
        ...
        def make_batch_iterator(dataloader):  # (CW) Use with IterableDataset.
            ...
            while True:
                ...
                for idx,batch in enumerate(dataloader):
                    ...
                    eeg_signal = reshape_eeg_signal(batch['eeg_signal']) # (CW)

                    if True:
                        print("Clipping input at +/-1")
                        eeg_signal.clamp(min=-1, max=1) # 
        ...
        # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
        #
        # (1). Here, inspect the eeg data to make sure nothing pathological is causing grad norm and loss to explode.
        if False:

            with torch.no_grad():

                start_batch = 0     # use these when not running on wole dataset. Otherwise 0 to start at beginning.
                end_batch = 1000       # use these when not running on wole dataset. Otherwise (992252//args.data.batch_size) to finish at end.

                okgo=True
                cnt=-1 
                elim = 3 # EEG limit to use for clipping normalized EEG signal.
                while okgo:
                    cnt+=1
                    ...
                    if True:
                        ...
                        ## Skip ahead to where the grad norm blows up.
                        if cnt < start_batch: # (24806 is whole dataset, 992252/40)
                            continue
                        ...
                        # Plot EEG Sample: Loop over samples in batch and plot EEG, FFT - compute PSD & PLV
                        if True:
                            N=range(args.data.batch_size)
                            k=args.data.batch_size # 5
                            samps = random.sample(N, k) #  k random samples from batch
                            for samp in samps: # range(eeg_signal.shape[0]):
                                signal_var = eeg_signal[samp].detach().numpy()  # 5 seconds x B channels of 256 Hz EEG data

                                # Compute Power Spectral Density within 5 canonical EEG bands - to measure single channel frequency content.
                                eeg_PSD, band_PSD = compute_eeg_PSD(signal_var, fs, 5)

                                # Compute Phase Locking Value between channels - to measure how pairs co-vary.
                                max_lag = 9
                                PLV_lags = list(range(-max_lag,max_lag+1)) #[0]
                                PLV_chans, PLV_best_lags = compute_plv_matrix(signal_var, PLV_lags)

                                # Plot EEG, FFT, PSD, PLV on one figure.
                                plot_eeg_fft_psd_plv(signal_var, 
                                                     fs, 
                                                     eeg_PSD, 
                                                     band_PSD, 
                                                     PLV_chans, 
                        ...
                        assert abs(eeg_signal.mean().item())  < 0.1         # mean is close to 0
                        # assert abs(eeg_signal.std().item() - 1) < 0.1       # std is close to 1
                        assert abs(eeg_signal.max().item()) <= elim         # max is bounded/reasonable
                        assert abs(eeg_signal.min().item()) <= elim         # min is bounded/reasonable

                        print(f"{eeg_signal.std().item()=}")

                        okgo=True
                        if cnt >= end_batch: # (992252//args.data.batch_size):
                            print('Data is looking just fine.')
                            okgo = False

                    # except:
                    #     print('Error: Hmm, data is non-canonical')
                    #     okgo=False



            import IPython; print('\n\n\Debug:'); IPython.embed(); import time;  time.sleep(0.3)
        # NOTE: THIS WHOLE SECTION WAS REPLACED WITH inspect_eeg_data.py  DELETE IT.
        ...
        while True:
            ...
            model_output = dec_out.cpu().float().numpy()
            ...
            if batch_idx > 50:
                ...
                data_dir   = Path(args.data.data_dir).expanduser().resolve()
                assert data_dir.name == "moabb", "expected the directory itself to be named 'moabb'"
                meta_dir = data_dir.with_name("moabb_meta")
                pt_files = sorted(meta_dir.glob("*.pt"))   # or data_dir.rglob("*.pt") for sub-dirs
                if not pt_files:
                    raise FileNotFoundError(f"No .pt files found in {data_dir}")
                first_pt_path = pt_files[0]
                metadata = torch.load(first_pt_path, map_location="cpu")   # or "cuda" if desired
                labels = metadata['labels']
                labels = labels[:len(model_output_arr)]
                class_mapping = metadata['class_mapping']
                
                channel_std = model_input_arr.std(axis=(0, 1))
                model_input_arr = model_input_arr[:, : , channel_std > 1e-6]
                model_output_arr = model_output_arr[:, :, channel_std > 1e-6]
                
                clf = LogisticRegression(penalty="l2",solver="saga",max_iter=300,n_jobs=-1,tol=1e-3)
                pipeline = make_pipeline(Covariances("oas"),TangentSpace(metric="riemann"),clf)

                X_train, X_test, y_train, y_test = train_test_split(model_input_arr, labels, test_size=0.2, random_state=42)
                pipeline.fit(X_train, y_train)
                y_pred = pipeline.predict(X_test)
                print(classification_report(y_test, y_pred, target_names=[class_mapping[i] for i in sorted(class_mapping)]))

                X_train, X_test, y_train, y_test = train_test_split(model_output_arr, labels, test_size=0.2, random_state=42)
                pipeline.fit(X_train, y_train)
                y_pred = pipeline.predict(X_test)
                print(classification_report(y_test, y_pred, target_names=[class_mapping[i] for i in sorted(class_mapping)]))
